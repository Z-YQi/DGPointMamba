...
class Metrics(object):
    ...
    },
    {
        'name': 'UCD',
        'enabled': True,
        'eval_func': 'cls._get_ucd_distance',
        'eval_object': UnidirectionalChamferDistance(ignore_zeros=True),
        'is_greater_better': False,
        'init_value': 32767
    },
    {
        'name': 'UHD',
        'enabled': True,
        'eval_func': 'cls._get_uhd_distance',
        'eval_object': DirectedHausdorffDistance(),
        'is_greater_better': False,
        'init_value': 32767
    }
    ...
    def _get_chamfer_distancel1(cls, pred, gt):
        ...
        return chamfer_distance(pred, gt) * 10000
    ...
    def _get_emd_distance(cls, pred, gt, eps=0.005, iterations=100):
        ...
        return emd_out * 10000
    
    @classmethod
    def _get_ucd_distance(cls, partial, pred, eps=0.005, iterations=100):
        ucd_distance = cls.ITEMS[4]['eval_object']  # 如果你有一个专门的UCD对象，比如在ITEMS里定义的
        return ucd_distance(partial, pred)*10000\
        
    @classmethod
    def _get_uhd_distance(cls, partial, pred, eps=0.005, iterations=100):
        partial = partial.permute([0,2,1])
        pred = pred.permute([0,2,1])
        uhd_distance = cls.ITEMS[5]['eval_object']  # 如果你有一个专门的UCD对象，比如在ITEMS里定义的
        return uhd_distance(partial, pred)*100
