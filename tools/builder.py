...
def virtual_dataset_builder(args, config):  
    ...
    if config.virtual_dataset in ['ScanNet', 'MatterPort']:
        args.split = 'trainval'
    elif config.virtual_dataset in ['ModelNet', '3D_FUTURE', 'KITTI', 'CRN']:
        args.split='train'
    if config.virtual_dataset in ['MatterPort','ScanNet','KITTI','PartNet']:
    ...
    if args.distributed:
        ...
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.others.bs if shuffle else 1,
                                                shuffle = shuffle, 
                                                drop_last = config._base_.SPLIT == 'train',
                                                num_workers = int(args.num_workers),
                                                worker_init_fn=worker_init_fn)
